...
function UserModel(params, callback, session) {
	Model.call(this, params, callback, session);
    
    var user = session.user;
    //console.log(user);

    this.register = function(params) {
    	session.register(params[0].uname, params[0].pword, params[0].email, function(user) {
            callback(user);
        });
	}
    
    this.login = function(params) {
        session.login(params[0].uname, params[0].pword, function(user) {
            callback(user);
        });
	}
    ...
    this.password = function(params) {
        session.changePassword(params[0].opword, params[0].npword, function(data) {
            callback(data);
        });
    }
    
    this.forgotpassword = function(params) {
        session.forgotPassword(params[0].uname, function(data) {
            callback(data);
        });
    }
