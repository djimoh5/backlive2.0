...
void Matrix(const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    for (size_t i = 0; i < len; i++) {
        //std::printf("%d %f\n", i, (*activation)[i] + (*inActivation)[i]);
        buffer[i] = (*activation)[i] + (*inActivation)[i];
        //output[i] = (*activation)[i] + (*inActivation)[i];
        //Nan::Set(results, i, Nan::New<v8::Number>((*activation)[i] + (*inActivation)[i]));
    }

    std::printf("%f ms\n", float(clock() - start));

    //Nan::NewBuffer(result, len)
    //info.GetReturnValue().Set(output);
    //free(result);
}

void Init(v8::Local<v8::Object> exports) {  
    exports->Set(Nan::New("matrix").ToLocalChecked(),
                 Nan::New<v8::FunctionTemplate>(Matrix)->GetFunction());
}
