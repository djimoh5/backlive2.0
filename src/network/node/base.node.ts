...
export abstract class BaseNode<T extends Node> {
    ...
    constructor(node: T, serviceType?: new(session: ISession, nodeRepository?) => NodeService<any>) {
        if(serviceType) {
    ...
    backpropagateMatrix(delta: Activation, state: State, activationError: ActivationError) {
        aoA.backpropagate(Buffer.from(<ArrayBuffer>delta.data().buffer), state.activation.data(), activationError.error.data(), activationError.weights,
    ...
    updateWeights() {
        ...
        if(this.node.weights) {
            ...
            aoA.updateWeights(Network.network.learnRate, this.learningError.trainingCount, this.numNodes, wlen, Buffer.from(<ArrayBuffer>this.node.weights.buffer), 
                Buffer.from(<ArrayBuffer>this.node.bias.buffer), this.learningError.total, this.learningError.totalBias, Network.network.regParam ? Network.network.regParam : 0);
            this.resetError();
