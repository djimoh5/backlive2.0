...
module.exports = function parser(file, options, callback) {
  ...
  function replaceFrag(callback) {
    ...
    if (fnIndex > -1 && opts.templateFunction) {
        ...
        frags.forEach(function (frag) {
            _urls.push(opts.templateFunction(/'(.*)'/.exec(frag)[1], opts.type, frag));
        });
    } else {
    ...
    urls.forEach(function (url) {
      ...
      if (HTML && opts.templateProcessor) {
        appendAssetFiles(opts.templateProcessor(ext, file));
      }
      else if (CSS && opts.styleProcessor) {
        opts.styleProcessor(ext, file, function(newFile) {
            appendAssetFiles(newFile);
        });
      }
      else {
        appendAssetFiles(file);
      }
    });
    
    function appendAssetFiles(file) {
        assetFiles += file;
        if(--numUrls === 0) {
            finish();
        }
